...
// Révélation de l'ananas après l'explosion des lettres
const REVEAL_DELAY_AFTER_SCATTER = -200; // démarre juste avant la fin de la dispersion
const REVEAL_DUR = 600;   // ms pour que l'ananas surgisse
const REVEAL_HOLD = 900;  // ms où l'ananas reste affiché avant onDone

// Moment où toutes les lettres ont fini de se disperser.
const SCATTER_END =
...
// Durée totale de la séquence (sert au timer JS qui déclenche onDone).
// Sans image de révélation, elle s'arrête à la fin de la dispersion.
const TOTAL_DURATION = SCATTER_END;
const TOTAL_DURATION_WITH_REVEAL =
  SCATTER_END + REVEAL_DELAY_AFTER_SCATTER + REVEAL_DUR + REVEAL_HOLD;

interface Props {
  onDone?: () => void;
  fontSize?: number;
  color?: string;
  /** Image révélée après l'explosion des lettres (ex. l'ananas). */
  revealSource?: ImageSourcePropType;
  revealSize?: number;
}

/**
 * Image qui surgit (scale 0 → 1 avec rebond + fondu) une fois les lettres
 * dispersées. Purement Reanimated côté UI, aucun callback worklet → JS.
 */
function RevealImage({
  ...
  const delay = SCATTER_END + REVEAL_DELAY_AFTER_SCATTER;

  useEffect(() => {
    opacity.value = withDelay(
      delay,
      withTiming(1, { duration: REVEAL_DUR * 0.6, easing: Easing.out(Easing.cubic) })
    );
    scale.value = withDelay(
      delay,
      withTiming(1, { duration: REVEAL_DUR, easing: Easing.out(Easing.back(1.6)) })
    );
  }, []);
  ...
  return (
    <Animated.Image
      source={source}
      resizeMode="contain"
      style={[styles.reveal, animStyle, { width: size, height: size }]}
    />
  );
}
...
export function AnimatedYouMe({
  ...
  useEffect(() => {
    if (!onDone) return;
    const total = revealSource ? TOTAL_DURATION_WITH_REVEAL : TOTAL_DURATION;
    const timer = setTimeout(onDone, total + 80);
  ...
  return (
    <View style={styles.container}>
      ...
      {revealSource ? <RevealImage source={revealSource} size={revealSize} /> : null}
    </View>
  );
}
