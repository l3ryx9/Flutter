...
export function useAuth() {
  ...
  const register = useCallback(
    async (
      ...
      consent: { analysisConsentAt: string; analysisConsentVersion: string }
    ): Promise<void> => {
      setLoading(true);
      clearError();
      try {
        ...
        try {
          newUser = await userRepository.createUser({
            ...
            analysisConsentAt: consent.analysisConsentAt,
            analysisConsentVersion: consent.analysisConsentVersion,
          });
        } catch (e: any) {
