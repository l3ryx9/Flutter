...
import { applyGlobalFont } from '../src/shared/utils/applyGlobalFont';
...
import { buildPaperTheme } from '../src/shared/constants/theme';
...
export default function RootLayout() {
  ...
  const { isDarkMode, themeId, loadPersistedState } = useUIStore();
  const theme = React.useMemo(() => buildPaperTheme(themeId, isDarkMode), [themeId, isDarkMode]);

  // ── Police Inter (gras) — appliquée à toute l'application ──────────────
  // Une fois chargée, on la propage à tous les <Text> et <TextInput>
  // natifs (voir applyGlobalFont), plutôt que de modifier chaque style
  // un par un : ça garantit une lisibilité homogène partout.
  const [fontsLoaded, fontError] = useFonts({ Inter_700Bold });

  useEffect(() => {
    if (!fontsLoaded) return;
    applyGlobalFont('Inter_700Bold');
  }, [fontsLoaded]);
